...
func (s *InstanceResourceCrud) Update() (e error) {
	...
	if !s.D.HasChange("create_vnic_details") {
		log.Printf("[DEBUG] No changes to primary VNIC. InstanceID: %q", s.Resource.ID)
		return
	}

	log.Printf("[DEBUG] Updating instance's primary VNIC. InstanceID: %q", s.Resource.ID)
	vnic, e := s.getPrimaryVnic()
	if e != nil {
		log.Printf("[ERROR] Primary VNIC could not be found during instance update: %q (InstanceID: %q, State: %q)", e, s.Resource.ID, s.Resource.State)
		return
	}
...
func (s *InstanceResourceCrud) SetData() {
	...
	if vnicError != nil {
		log.Printf("[WARN] Primary VNIC could not be found during instance refresh: %q (InstanceID: %q, State: %q)", vnicError, s.Resource.ID, s.Resource.State)
		return
	}
